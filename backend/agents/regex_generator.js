...
Make sure the regex is optimized and the test cases cover both matching and non-matching scenarios. Include at least 4 test cases. Provide code examples for at least 2 programming languages.`;
    ...
    try {
      ...
      parsedResponse = {
        regex: extractRegexFromText(responseText),
        flags: extractFlagsFromText(responseText),
        explanation: extractExplanationFromText(responseText),
        test_cases: extractTestCasesFromText(responseText),
        code_examples: [
          {
            language: "JavaScript",
            code: `const regex = /${extractRegexFromText(responseText)}/${extractFlagsFromText(responseText)};\nconsole.log(regex.test("test string"));`,
            description: "Basic usage in JavaScript"
          },
          {
            language: "Python",
            code: `import re\npattern = r"${extractRegexFromText(responseText)}"\nresult = re.search(pattern, "test string")\nprint(result)`,
...
function extractRegexFromText(text) {
